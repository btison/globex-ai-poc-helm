...
{{- define "minio.root.username" -}}
  ...
  {{- else }}
    {{- include "minio.getValueFromSecret" (dict "Namespace" .Release.Namespace "Name" (include "minio.name" .) "Length" 20 "Key" "rootUser") }}
  {{- end }}
{{- end -}}
...
{{- define "minio.root.password" -}}
  ...
  {{- else }}
    {{- include "minio.getValueFromSecret" (dict "Namespace" .Release.Namespace "Name" (include "minio.name" .) "Length" 40 "Key" "rootPassword") }}
  {{- end }}
{{- end -}}
...
ArgoCD Syncwave
*/}}
{{- define "minio.hook.argocd-syncwave" -}}
{{- if .Values.argocd }}
{{- if and (.Values.argocd.hook.syncwave) (.Values.argocd.enabled) -}}
argocd.argoproj.io/sync-wave: "{{ .Values.argocd.hook.syncwave }}"
{{- else }}
{{- "{}" }}
{{- end }}
{{- else }}
